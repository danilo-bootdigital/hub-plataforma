...
export function PainelResultado({ detalhe: v, onDecidido }: { detalhe: Detalhe; onDecidido?: () => void }) {
    ...
    <div className="space-y-4">
      ...
      {v.status_processamento === 'erro' && (
        <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          <AlertTriangle className="size-4 shrink-0" /> Falha ao processar a análise. Crie uma nova validação.
        </div>
      )}
      ...
      {/* Comparar Posologia — consultivo, sob demanda (opção B) */}
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        {v.podeAprovar
          ? <AcoesDecisao conferenciaId={v.id} onDecidido={onDecidido} />
          : <p className="text-sm text-slate-400">Somente o Proprietário do Hub pode registrar a decisão.</p>}
      </div>

      {/* Dados agrupados (referência para o conferente) */}
      ...
      <details className="rounded-lg border border-slate-200 bg-white">
        <summary className="cursor-pointer px-4 py-2.5 text-sm font-medium text-slate-500 hover:text-slate-700">Informações técnicas</summary>
